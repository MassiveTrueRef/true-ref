...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`navigation_links\` (
  	...
  	\`url\` text,
  	FOREIGN KEY (\`reference_id\`) REFERENCES \`pages\`(\`id\`) ON UPDATE no action ON DELETE set null,
  ...
  await db.run(sql`CREATE INDEX \`navigation_links_reference_idx\` ON \`navigation_links\` (\`reference_id\`);`)
